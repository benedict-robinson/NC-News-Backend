    ...
    test("GET: 200 - topics are sorted by slug ASC", () => {
      return request(app)
        ...
        .then(({ body }) => {
          ...
          expect(lettersIndex.reverse()).toEqual(
            lettersPreSort.sort((a, b) => b - a)
          );
        });
    });
  });
  describe("POST /api/topics", () => {
    ...
    test("POST: 201 - adds the topic to the topic table", () => {
      ...
      return request(app)
        ...
        .then(() => {
          return db
            .query(`SELECT * FROM topics WHERE slug = 'Dutch Cheese';`)
            .then(({ rows }) => {
...
describe("Articles", () => {
  describe("GET /api/articles", () => {
    ...
    describe("GET articles - queries", () => {
      ...
      test("GET: 400 - responds with 400 Bad Request when a query is given that is invalid", () => {
        return request(app)
          .get("/api/articles?not-a-query=title")
          .expect(400)
          .then(({ body }) => {
      ...
      test("GET: 400 - responds with 400 Bad Request when a query is given that is not accepted (order)", () => {
        return request(app)
          .get("/api/articles?order=not-a-query")
          .expect(400)
          .then(({ body }) => {
            expect(body.msg).toBe("Bad Request - Invalid Query");
          });
      ...
      test("GET: 400 - responds with 400 Bad Request when a query is given that is not accepted (sort_by)", () => {
        return request(app)
          .get("/api/articles?sort_by=not-a-query")
          .expect(400)
          .then(({ body }) => {
            expect(body.msg).toBe("Bad Request - Invalid Query");
          });
      ...
      test("GET: 404 - responds with 404 Not Found when given a valid but non-existent topic query", () => {
        return request(app)
          .get("/api/articles?topic=dutch_cheese")
          .expect(404)
          .then(({ body }) => {
      ...
      test("GET: 200 - all queries can be chained", () => {
        return request(app)
          .get(
            "/api/articles?author=rogersop&topic=mitch&sort_by=title&order=desc"
          )
          .expect(200)
          .then(({ body }) => {
            expect(body.articles).toHaveLength(2);
    ...
    test("POST: 201 - adds the article to the article table", () => {
      ...
      return request(app)
        ...
        .then(() => {
          return db
            .query(
              `SELECT * FROM articles WHERE title = 'Paper Cuts - owwy owwy owwy';`
            )
            .then(({ rows }) => {
  ...
  describe("/api/articles/:article_id", () => {
    ...
    test("Comment Count - responds get: 200 and lists the total number of comments on the selected article", () => {
      return request(app)
        .get("/api/articles/6?comment_count")
        .expect(200)
        .then(({ body }) => {
          const articleSix = {
            article_id: 6,
          ...
          return db
            ...
            .then(({ rows }) => {
              ...
              expect(rows[0].body).toBe(
                "Is this the same Mitch that runs the seminars?"
              );
            });
        });
    });
...
describe("Users", () => {
  describe("GET Users", () => {
    ...
    describe("Get Users - Queries", () => {
      ...
      test("GET: 404 - responds with a User Not Found if username given is valid but doesn't exist", () => {
        return request(app)
          .get("/api/users?username=brigadier_benno")
          .expect(404)
          .then(({ body }) => {
            expect(body.msg).toBe("User Not Found");
          });
  ...
  describe("GET Comments by User", () => {
    ...
    test("GET: 404 - responds with 404 Not Found when passed a non-existent user", () => {
      return request(app)
        .get("/api/users/not-a-user/comments")
        .expect(404)
        .then(({ body: { msg } }) => {
